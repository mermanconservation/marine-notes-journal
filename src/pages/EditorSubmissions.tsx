                  ...
                  {selectedSub.pipeline_status && selectedSub.pipeline_status !== 'pending' && (
                    <>
                      ...
                      <div>
                        ...
                        {/* Download Report & Publish actions */}
                          ...
                          {selectedSub.pipeline_results?.steps && (
                            <Button
                              ...
                              onClick={() => {
                                ...
                                const results = sub.pipeline_results;
                                const scores = results.prepared_metadata?.pipeline_scores;
                                let report = `AI REVIEW PIPELINE — ANALYTICAL REPORT\n`;
                                report += `${"=".repeat(50)}\n\n`;
                                report += `Title: ${sub.title}\n`;
                                report += `Authors: ${sub.all_authors}\n`;
                                report += `Type: ${sub.manuscript_type}\n`;
                                report += `Submission Date: ${formatDate(sub.created_at)}\n`;
                                report += `Pipeline Status: ${sub.pipeline_status.toUpperCase()}\n`;
                                report += `Average Score: ${scores?.average ?? "N/A"}/100\n\n`;
                                report += `${"─".repeat(50)}\n\n`;

                                results.steps.forEach((step: any, i: number) => {
                                  report += `STEP ${i + 1}: ${step.step.replace(/_/g, " ").toUpperCase()}\n`;
                                  report += `Result: ${step.passed ? "PASSED ✓" : "FAILED ✗"}  |  Score: ${step.score}/100\n\n`;
                                  report += `Summary:\n${step.summary}\n\n`;
                                  if (step.issues.length > 0) {
                                    report += `Issues Found:\n`;
                                    step.issues.forEach((issue: string) => {
                                      report += `  • ${issue}\n`;
                                    });
                                  }
                                  report += `\n${"─".repeat(50)}\n\n`;
                                ...
                                if (results.rejection_reasons?.length > 0) {
                                  report += `REJECTION REASONS:\n`;
                                  results.rejection_reasons.forEach((r: string) => {
                                    report += `  • ${r}\n`;
                                  });
                                  report += `\n`;
                                }

                                report += `\nGenerated by Marine Notes Journal AI Review Pipeline\n`;
                                ...
                                const a = document.createElement("a");
                                a.href = url;
                                const slug = sub.title.replace(/[^a-zA-Z0-9\s-]/g, "").trim().replace(/\s+/g, "-").substring(0, 60);
                                a.download = `AI-Review-Report-${slug}.txt`;
                                document.body.appendChild(a);
                                a.click();
                                document.body.removeChild(a);
                                URL.revokeObjectURL(url);
                            ...
                            >
                              <Download className="h-3 w-3 mr-1" /> Download Review Report
                            </Button>
                          )}
